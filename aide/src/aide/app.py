...
# --- Global State ---
tools = [read_file_tool, write_file_tool, command_runner_tool]
...
def spec_generator(user_input):
    ...
    try:
        ...
        print("Specification written to spec.json")
        return spec
    except json.JSONDecodeError:
        print("Error: Spec generator did not return valid JSON.")
        return None
...
def planner(spec):
    ...
    try:
        ...
        print("Plan written to plan.json")
        return plan
    except json.JSONDecodeError:
        print("Error: Planner did not return valid JSON.")
        return None

def implementer(spec, plan, critic_feedback=""):
...
def tester(spec):
    with open("aide/prompts/tester_prompt.txt", "r") as f:
        prompt = f.read().format(spec=json.dumps(spec, indent=4))
    test_report_json = run_agent_turn(prompt)
    try:
        if test_report_json.startswith("```json"):
            test_report_json = test_report_json[7:-4]
        test_report = json.loads(test_report_json)
        with open("test_report.json", "w") as f:
            json.dump(test_report, f, indent=4)
        print("Test report written to test_report.json")
        return test_report
    except json.JSONDecodeError:
        print("Error: Tester did not return valid JSON.")
        return None

def critic(spec, plan, test_report):
    code = ""
    ...
    try:
        ...
        print("Error: Critic did not return valid JSON.")
        return None
...
def main():
    ...
    for i in range(3): # Max 3 iterations
        ...
        implementer(spec, plan, critic_feedback)
        
        test_report = tester(spec)
        if not test_report: return 1

        # Check for success
        if "error" not in json.dumps(test_report).lower() and "fail" not in json.dumps(test_report).lower():
            print("\n--- IMPLEMENTATION SUCCESSFUL ---")
            break
            
        critic_feedback = critic(spec, plan, test_report)
